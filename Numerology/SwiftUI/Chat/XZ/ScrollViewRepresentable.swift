...
struct ScrollViewRepresentable<Content: View>: UIViewRepresentable {
    ...
    class Coordinator: NSObject, UIScrollViewDelegate {
        ...
        init(_ parent: ScrollViewRepresentable) {
            self.parent = parent
            //            myPrint("🔹init with contentOffset.x",scrollView?.contentOffset.x)
        }
        ...
        func scrollViewDidScroll(_ scrollView: UIScrollView) {
            ...
            myPrint("🌕", scrollView.contentOffset.x / scrollView.frame.size.width)
        }
        
        func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
            self.parent.canSetPage = true
            myPrint("🟢 scrollViewDidEndDecelerating",self.parent.canSetPage)
        }
        
        func setPage(_ page: Int, animated: Bool = true) {
            //            myPrint("🟣setPage_v2")
            
//            guard let scrollView = self.scrollView else { myPrint("❌ not get scrollView"); return }
//            
