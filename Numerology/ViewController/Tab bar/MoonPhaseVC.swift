...
class MoonPhaseVC: UIViewController {
    ...
    private let accordionView: AccordionView = {
        let v = AccordionView()
        v.showAccordion()
        v.accordionButton.configure(title: "Generally")
        v.info.text = "test info"
        v.about.text = "test about"
        return v
    }()
...
//        print("⚠️⚠️⚠️",makeStringDay(strDate: "2024-02-01 18:06:46 +0000")?.get(.day))
        HoroscopeManager.shared.getMoonPhase(currentDay: 12) { model in
            print("⚠️⚠️⚠️",model.moonDay)
            print("⚠️⚠️⚠️",model.todayInfo)
        }
    }
    func makeStringDay(strDate: String) -> Date? {
